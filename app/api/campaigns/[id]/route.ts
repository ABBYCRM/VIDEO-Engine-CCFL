...
export async function PATCH(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  db.prepare(`UPDATE campaigns SET name=?,mission=?,video_provider=?,video_model=?,upper_provider=?,upper_model=?,split_percent=?,split_relationship=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(
    name,mission,surface.videoProvider,surface.videoModel,surface.upperProvider,surface.upperModel,surface.splitPercent,surface.splitRelationship,id
  );
  return NextResponse.json({campaign:db.prepare(`SELECT ${CAMPAIGN_SELECT} FROM campaigns WHERE id=?`).get(id)});
}
