...
export async function GET(){if(!(await requireAdmin()))return NextResponse.json({error:"Unauthorized"},{status:401});const campaigns=db.prepare(`SELECT id,name,category,website,mission,tone,platform,target_audience as targetAudience,avatar_id as avatarId,background_id as backgroundId,planning_horizon_days as planningHorizonDays,content_type as contentType,output_mode as outputMode,video_provider as videoProvider,status,created_at as createdAt,updated_at as updatedAt FROM campaigns ORDER BY created_at DESC`).all();return NextResponse.json({campaigns})}

export async function POST(req:Request){
  if(!(await requireAdmin()))return NextResponse.json({error:"Unauthorized"},{status:401});const body=await req.json().catch(()=>({})),cookieProvider=(await cookies()).get("ve_campaign_provider")?.value||"veo";
  const name=String(body.name||"").trim().slice(0,180),category=String(body.category||"").trim().slice(0,80),website=String(body.website||"").trim().slice(0,500),mission=String(body.mission||"").trim().slice(0,4000),tone=String(body.tone||"").trim().slice(0,200),platform=String(body.platform||"instagram").trim().slice(0,80),avatarId=String(body.avatarId||"").trim().slice(0,120),backgroundId=String(body.backgroundId||"").trim().slice(0,120),horizon=Number(body.planningHorizonDays||7),contentType=CONTENT_TYPES.has(String(body.contentType))?String(body.contentType):"cinematic",outputMode=OUTPUT_MODES.has(String(body.outputMode))?String(body.outputMode):"video",requestedProvider=String(body.videoProvider||cookieProvider),videoProvider=VIDEO_PROVIDERS.has(requestedProvider)?requestedProvider:"veo";
