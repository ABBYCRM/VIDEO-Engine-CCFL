...
export async function POST(req:Request){
  ...
  const id=crypto.randomUUID();db.prepare(`INSERT INTO campaigns(id,name,category,website,mission,tone,platform,avatar_id,background_id,planning_horizon_days,content_type,output_mode,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?, 'draft')`).run(id,name,category,website||null,mission,tone||null,platform||null,avatarId||null,backgroundId||null,horizon,contentType,outputMode);
  ...
  const campaign=db.prepare(`SELECT id,name,category,website,mission,tone,platform,avatar_id as avatarId,background_id as backgroundId,planning_horizon_days as planningHorizonDays,content_type as contentType,output_mode as outputMode,status,created_at as createdAt FROM campaigns WHERE id=?`).get(id);
