...
export async function POST(req:Request){
  ...
  try{
    ...
    const id=crypto.randomUUID(),ext=mime==="video/mp4"?"mp4":"webm",relative=`/generated/compositions/${id}.${ext}`,absolute=path.resolve(process.cwd(),"public",relative.slice(1)),bytes=Buffer.from(await file.arrayBuffer());await fs.mkdir(path.dirname(absolute),{recursive:true});await fs.writeFile(absolute,bytes);
    ...
    const persistentUrl=await savePersistentLibraryAsset({id:`composition:${id}`,kind:"composition",mediaType:"video",label:`Split-screen · ${split}% top`,title,mimeType:mime,bytes,model:"browser composition",prompt:caption,metadata:{upperSource:upper,lowerSource:lower,splitPercent:split}}).catch(()=>null);
    const mediaUrl=persistentUrl||relative;
    db.prepare("INSERT INTO generated_compositions(id,title,file_path,mime_type,upper_source,lower_source,split_percent) VALUES(?,?,?,?,?,?,?)").run(id,title,mediaUrl,mime,upper||null,lower||null,split);
    ensureAssetCalendarPost({sourceKey:`composition:${id}`,title,contentType:"podcast",mediaUrl,mediaType:mime,caption});
    return NextResponse.json({id,url:mediaUrl,mimeType:mime,title},{status:201});
