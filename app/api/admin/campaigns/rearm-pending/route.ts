...
import { startCampaignAutopilotLoop, runCampaignAutopilotOnce, rewriteUnpublishedCaptions } from "@/lib/campaign-autopilot";
import { ensureSplitSurfaceColumns } from "@/lib/split-surface";
import { ensureUpperVideoColumns } from "@/lib/upper-videos";
...
export async function POST(req:Request) {
  ...
  `;
  if (body.forceResetSplitScreen) {
    resetSplit = campaignId
      ? db.prepare(`${resetSql} AND campaign_id=?`).run(campaignId)
      : db.prepare(resetSql).run();
  } else if (body.resetSplitScreen) {
    resetSplit = campaignId
      ? db.prepare(`${resetSql} AND campaign_id=? AND (source_asset_key IS NULL OR source_asset_key NOT LIKE 'composition:%')`).run(campaignId)
      : db.prepare(`${resetSql} AND (source_asset_key IS NULL OR source_asset_key NOT LIKE 'composition:%')`).run();
  }
  const rewritten = body.rewriteCaptions === false ? 0 : rewriteUnpublishedCaptions(campaignId || undefined);
  startCampaignAutopilotLoop();
  const tick = await runCampaignAutopilotOnce();
  return NextResponse.json({ ok: true, rearmed: pendingManual.changes, retryableA2e: failedA2e.changes, resetSplit: resetSplit.changes, rewritten, processed: tick.processed });
}
