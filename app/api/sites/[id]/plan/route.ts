...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{const drafts=await generateBlogPlan(id,horizonDays);const calendarIds=drafts.map(d=>ensureAssetCalendarPost({sourceKey:`blog:${d.id}`,title:d.title,contentType:"blog",network:"website",caption:`${d.excerpt}\n\nOutline:\n${d.outline.map((x:string)=>`- ${x}`).join("\n")}\n\nImage prompt: ${d.imagePrompt}`.slice(0,5000),siteId:id,scheduledAt:new Date(d.scheduledAt)}));db.prepare("UPDATE sites SET updated_at=CURRENT_TIMESTAMP WHERE id=?").run(id);return NextResponse.json({ok:true,count:drafts.length,drafts,calendarIds},{status:201});}
