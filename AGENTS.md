...
Left-nav **Claw** is the operator agent (NVIDIA NIM, default `nvidia/nemotron-3-super-120b-a12b`, streaming with thinking off and native OpenAI tool calling). Same Grok-style chat chrome: new thread, delete thread/message, upload, rename, attach files. Tools call the same server functions as Create / Pipeline / Calendar / Library / Avatars / Sites / Instagram / Creator (`creator_upload_video` schedules a file attached in chat exactly like the old Creator tab did). Never dump secrets.
