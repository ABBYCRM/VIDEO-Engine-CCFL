...
import {
  ...
  Calendar, Plug, Library, Mic, Sparkles, Plus, Globe, ChevronDown, X
} from "lucide-react";
...
const NAV: NavItem[] = [
  ...
  { href: "/calendar",            label: "Calendar",      icon: Calendar,    group: "WORKFLOW",  badge: 13 },
  ...
  { href: "/components-demo",     label: "Components",    icon: Sparkles,    group: "ACCOUNT", badge: "new" },
  ...
  return (
    <div className="min-h-screen text-slate-900">
      ...
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-200 bg-white/90 px-4 py-2.5 backdrop-blur md:hidden">
        ...
        <button onClick={() => setMobileOpen(v => !v)} className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 bg-white text-slate-700">
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`fixed inset-y-0 left-0 z-20 w-64 border-r border-slate-200 bg-white transition-transform md:translate-x-0 ${
          ...
          <div className="flex h-full flex-col">
            <div className="border-b border-slate-200 p-3">
              <Link href="/" className="mb-3 flex items-center gap-2 px-1.5">
                ...
                <span className="ml-auto rounded-md border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-slate-500">
                  CCFL
                </span>
              </Link>
              <button
                type="button"
                className="group flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/50 px-2 py-1.5 text-left transition hover:bg-slate-100"
              >
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-white text-slate-500 ring-1 ring-slate-200">
                  <Globe size={14} />
                </span>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-medium text-slate-900">CaseClosedFL | Florida</div>
                  <div className="truncate text-[11px] text-slate-500">caseclosedfl.com</div>
                </div>
            ...
            <nav className="flex-1 overflow-y-auto px-2 pb-2">
                      ...
                      return (
                        <li key={n.href}>
                          <Link
                            ...
                            className={`group flex items-center gap-2 rounded-xl px-2.5 py-2 text-[14px] transition ${
                              active
                          ...
                          >
                            ...
                            {n.badge === "new" && <span className="soro-new-pill">new</span>}
                            {typeof n.badge === "number" && <span className="soro-count-pill">{n.badge}</span>}
                          </Link>
                        </li>
                      );
                    })}
              ...
              <button
                type="button"
                className="flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-[14px] text-slate-500 transition hover:bg-slate-50 hover:text-slate-700"
              >
                <Plus size={16} className="text-slate-600" />
                <span>Add more websites</span>
              </button>
            </nav>
            ...
            <div className="border-t border-slate-200 p-2">
              <div className="flex items-center gap-2 rounded-xl px-2 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-medium text-slate-900">PA</div>
        ...
        {mobileOpen && <div onClick={() => setMobileOpen(false)} className="fixed inset-0 z-10 bg-slate-100/30 md:hidden" />}
