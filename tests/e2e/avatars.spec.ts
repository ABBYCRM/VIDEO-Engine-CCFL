...
test("avatar image settings switch providers and refresh model choices", async ({ page }) => {
  ...
  await page.route("**/api/admin/avatars/image-settings", async route => {
    if (route.request().method() === "POST") {
      ...
      return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ configured: true, provider: input.provider, model: input.model, providers, modelChoices: input.provider === "openai" ? ["gpt-image-1"] : ["gemini-2.5-flash-image"] }) });
    }
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ configured: true, provider: "gemini", model: "gemini-2.5-flash-image", providers, modelChoices: ["gemini-2.5-flash-image"] }) });
  });
...
test("gemini spending cap failure is summarized and recoverable", async ({ page }) => {
  ...
  await page.route("**/api/admin/avatars", route => route.fulfill({
    ...
    body: JSON.stringify({ avatars: [{
      ...
      gender: "female",
      archetype: "newsroom spokesperson",
      wardrobeStandard: "tailored blazer / blouse / slacks",
      notes: "Professional newsroom anchor",
      referenceImage: "/avatars/female-anchor-01/identity.jpg",
      referenceImageNote: null,
      wardrobeRegenerationPrompt: null,
      status: "draft",
      turnaroundStatus: "failed",
      turnaroundModel: "gemini-2.5-flash-image",
      turnaroundError: failedView.generationError,
      views: { front: failedView, left: failedView, right: failedView, back: failedView }
    }] })
  }));
