...
test("podcast is an explicit Create mode instead of duplicate sidebar navigation",async({page})=>{await page.goto("/");await expect(page.getByRole("heading",{name:"Create"})).toBeVisible();await page.getByLabel("Content format").selectOption("podcast");const entry=page.getByRole("link",{name:"Continue to two-lane production"});await expect(entry).toBeVisible();await entry.click();await expect(page).toHaveURL(/\/podcast-interview/);await expect(page.getByRole("heading",{name:"Two-lane campaign production"})).toBeVisible();await openNavigationIfNeeded(page);await expect(page.getByRole("navigation").getByRole("link",{name:"Podcast / split-screen",exact:true})).toHaveCount(0)});
