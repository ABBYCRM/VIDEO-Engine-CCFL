...
test("podcast is an explicit Create mode instead of duplicate sidebar navigation",async({page})=>{await page.goto("/");const entry=page.getByRole("link",{name:"Open split-screen composer",exact:true});await expect(entry).toBeVisible();await entry.click();await expect(page).toHaveURL(/\/podcast-interview$/);await expect(page.getByRole("heading",{name:"Podcast / split-screen composer"})).toBeVisible();await openNavigationIfNeeded(page);await expect(page.getByRole("navigation").getByRole("link",{name:"Podcast / split-screen",exact:true})).toHaveCount(0)});
