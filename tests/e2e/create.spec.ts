...
test("create keeps all video engines selectable and Hedra exposes long-form inputs", async ({ page }) => {
  await stubBase(page); await page.goto("/");
  ...
  await page.getByRole("button", { name:/Hedra/i }).click(); await expect(page.getByLabel("Hedra duration")).toHaveValue("30"); await expect(page.getByText("Driving audio (required)")).toBeVisible(); await expect(page.getByRole("button", { name:"Generate 30s with Hedra" })).toBeDisabled();
});
...
test("successful campaign generation can enter approval queue or publish to Instagram", async ({ page }) => {
  ...
  await page.goto("/"); await page.getByLabel("Mission").fill("Explain why scene documentation matters."); await page.getByRole("button", { name:"Generate 8s with Google Veo 3.1" }).click();
  await expect.poll(async()=>page.getByRole("button",{name:"Send to approval Calendar"}).count(),{timeout:10000}).toBe(1); await page.getByRole("button", { name:"Send to approval Calendar" }).click(); await expect(page.getByRole("button", { name:"Added to Calendar" })).toBeVisible(); expect(calendarPayload.status).toBe("pending"); expect(calendarPayload.autoPost).toBe(false); expect(calendarPayload.videoJobId).toBe("job-create");
  page.on("dialog", dialog=>dialog.accept()); await page.getByRole("button", { name:"Post to Instagram now" }).click(); await expect(page.getByRole("button", { name:"Posted to Instagram" })).toBeVisible(); expect(instagramPayload.jobId).toBe("job-create");
});
