...
const BASE_APIS = async (page: import("@playwright/test").Page) => {
  ...
  await page.route("**/api/admin/avatars/image-settings", route => route.fulfill({ status:200, contentType:"application/json", body:JSON.stringify({ configured:true, provider:"gemini", model:"gemini-2.5-flash-image", providers:imageProviders, modelChoices:["gemini-2.5-flash-image"] }) }));
...
function rgb(value:string): [number,number,number] {const nums=value.match(/\d+(?:\.\d+)?/g)?.map(Number)||[0,0,0];return[nums[0]||0,nums[1]||0,nums[2]||0];}
...
test("deployed workflow pages expose the requested production UI", async ({ page }) => {
  ...
  const checks:[string,RegExp][]=[["/",/Create campaign content/i],["/avatars",/Avatars/i],["/sites",/^Sites$/i],["/library",/^Library$/i],["/calendar",/Content Calendar/i],["/campaigns",/^Campaigns$/i]];
  for(const [path,heading] of checks){await page.goto(path,{waitUntil:"domcontentloaded"});await expect(page.getByRole("heading",{name:heading})).toBeVisible({timeout:15000});}
});

test("deployed Create keeps providers and canonical avatar choice", async ({ page }) => {
  await BASE_APIS(page);await page.goto("/");
  ...
  await expect(page.getByText(/Choose canonical avatar/i)).toBeVisible();await expect(page.getByRole("option",{name:/Female Anchor 01/i})).toBeAttached();
});

test("deployed Avatars offers NVIDIA FLUX turnaround provider",async({page})=>{
  await BASE_APIS(page);await page.goto("/avatars");
  await page.getByRole("button",{name:/Image: gemini/i}).click();
  await expect(page.getByRole("button",{name:/NVIDIA FLUX.2 Klein 4B/i})).toBeVisible();
});

test("deployed Sites is blog and image focused with horizon planning", async ({ page }) => {
  await BASE_APIS(page);await page.goto("/sites");
  await expect(page.getByText(/Blog \+ images only/i)).toBeVisible();await page.getByRole("button",{name:/Add site/i}).click();
  await expect(page.getByLabel(/CMS \/ publishing target/i)).toBeVisible();await expect(page.getByLabel(/Image style/i)).toBeVisible();await expect(page.getByText(/No video settings exist in this workflow/i)).toBeVisible();
});

test("deployed Calendar exposes approval manual publish auto-post and generated assets", async ({ page }) => {
  await BASE_APIS(page);await page.goto("/calendar");
  await expect(page.getByText(/Generated blog articles, images and videos land here automatically/i)).toBeVisible();
  await expect(page.getByRole("button",{name:/Add post/i})).toBeVisible();await expect(page.getByText(/Owner review queue/i)).toBeVisible();
});

test("deployed shared text buttons meet WCAG AA contrast", async ({ page }) => {
  await BASE_APIS(page);
  for(const path of ["/","/sites","/library","/calendar","/avatars","/campaigns"]){await page.goto(path);const buttons=page.locator("button:not([disabled])");for(let i=0;i<await buttons.count();i++){const b=buttons.nth(i);if(!(await b.isVisible()))continue;const text=(await b.innerText()).trim();if(!text)continue;const s=await b.evaluate(el=>{const x=getComputedStyle(el);return{fg:x.color,bg:x.backgroundColor}});if(s.bg==="rgba(0, 0, 0, 0)"||s.bg==="transparent")continue;expect(ratio(rgb(s.fg),rgb(s.bg)),`${path}: ${text}`).toBeGreaterThanOrEqual(4.5);}}
});
