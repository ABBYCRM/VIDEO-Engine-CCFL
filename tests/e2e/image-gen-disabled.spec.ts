...
test("image-gen disabled state is correct on the live build", async ({ page, request }) => {
  ...
  // Must include kept items. "/" (Create) and "/creator" (Creator) are
  // intentionally not nav links anymore - Claw's generate_video/
  // generate_still/ugc_batch_generate/creator_upload_video tools already
  // cover what those pages did, so both were dropped from the left rail
  // (both routes still redirect to /calendar, checked separately above as
  // "kept pages").
  for (const kept of ["/calendar", "/library", "/settings", "/claw"]) {
  ...
  expect(navItems, "nav should NOT include /creator (Creator, retired)").not.toContain("/creator");
