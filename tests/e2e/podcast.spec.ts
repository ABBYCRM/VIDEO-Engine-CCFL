...
test("split-screen still supports uploaded upper video and a selectable frame template",async({page})=>{await stubAuthenticatedSession(page);await page.route("**/api/admin/avatars",route=>route.fulfill({status:200,contentType:"application/json",body:JSON.stringify({avatars:[]})}));await page.goto("/podcast-interview");await page.locator('input[type="file"][accept="video/*"]').first().setInputFiles({name:"context.mp4",mimeType:"video/mp4",buffer:Buffer.from("fake-video")});await expect(page.locator("video").first()).toBeVisible();await expect(page.locator('img[src="/backgrounds/split-template-office-modern.png"]')).toBeVisible();await page.getByLabel("Split-screen template").selectOption("office-warm");await expect(page.locator('img[src="/backgrounds/split-template-office-warm.png"]')).toBeVisible();});
