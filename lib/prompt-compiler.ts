...
export function compileVeoPrompt(input: { category: CampaignCategory; mission?: string; script?: string; subject?: string }) {
  const template = campaignTemplates[input.category];
  const dialogue = input.script?.trim() ? `Dialogue, spoken exactly and naturally: ${JSON.stringify(input.script.trim())}.` : "If speech is useful, keep it brief, conversational, legally cautious, and non-promissory.";
  const prompt = [
