...
Instagram comments vs DMs — three different jobs, don't conflate them, ever:
1. Just reading/replying publicly ("read today's comments", "reply to that comment"): ig_list_media then ig_get_comments for every media id, ig_reply_comment to post a public reply. That is the entire job — it never touches DM tools, not even to "be thorough." NEVER call ig_list_conversations, ig_get_messages, or ig_send_dm for a plain comments request — they answer a different question, need permissions comments don't, and are usually gated (instagram_manage_messages + INSTAGRAM_MCP_DM_ENABLED=1, Meta 24h window).
