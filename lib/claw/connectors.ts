...
export function connectorInventory() {
  ...
  return {
    ...
    gemini: { configured: Boolean(secret("gemini_api_key", "GEMINI_API_KEY")), when: "OPTIONAL alternate: gemini_generate only when the operator names Gemini. Does not replace Bitdeer Claw chat/vision." },
    xai: { configured: Boolean(secret("xai_api_key", "XAI_API_KEY")), when: "OPTIONAL alternate: xai_chat when the operator names xAI/Grok chat. Not the Claw runtime." },
    kimi: { configured: Boolean(secret("kimi_api_key", "KIMI_API_KEY")), when: "OPTIONAL alternate: kimi_chat when the operator names Kimi/Moonshot. Not the Claw runtime." },
    openai: { configured: Boolean(firstSecret([["openai_api_key", "OPENAI_API_KEY"], ["openai_embeddings_api_key", "OPENAI_EMBEDDINGS_API_KEY"], ["openai_embeddings", "OPENAI_EMBEDDINGS"]])), when: "OPTIONAL alternate: openai_chat / openai_embed. Bitdeer remains default embed/rerank/chat." },
