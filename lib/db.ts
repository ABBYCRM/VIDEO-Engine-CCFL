...
CREATE TABLE IF NOT EXISTS avatars (
  ...
  -- 4-view turnaround status: draft (no reference image), generating
  -- (at least one view is being generated by AI), incomplete (some views
  -- ready, others missing), ready (all 4 views ready), failed (last
  -- generation attempt failed and was not retried)
...
CREATE TABLE IF NOT EXISTS avatar_views (
  ...
  -- Per-view generation state: idle (not yet), generating (AI in flight),
  -- ready (saved), failed (last attempt failed)
