...
export function getJob(id: string): VideoJob | undefined {
  return db.prepare(
    "SELECT id,source,category,prompt,provider,model,aspect_ratio as aspectRatio,resolution,provider_operation as providerOperation,status,error,output_path as outputPath,created_at as createdAt,updated_at as updatedAt FROM video_jobs WHERE id=?"
  ).get(id) as VideoJob | undefined;
}

export async function ensureJobOutputPath(id: string) {
  const job = getJob(id);
  if (!job || job.status !== "succeeded" || !job.outputPath) return null;
  try {
...
export async function refreshJob(id: string) {
  ...
  try {
    ...
    else if (job.provider === "hedra") result = await hedra.pollOneShot(job.providerOperation, id);
    ...
    db.prepare("UPDATE video_jobs SET status='failed',error=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(providerError(job.provider, error), id);
  }
  return getJob(id);
}

export function listRecentJobs(limit = 20): VideoJob[] {
  return db.prepare(
    "SELECT id,source,category,prompt,provider,model,aspect_ratio as aspectRatio,resolution,provider_operation as providerOperation,status,error,output_path as outputPath,created_at as createdAt,updated_at as updatedAt FROM video_jobs ORDER BY created_at DESC LIMIT ?"
  ).all(limit) as VideoJob[];
}

export function videoOutputDir() {
  return path.resolve(process.env.VIDEO_OUTPUT_DIR || "./data/videos");
}
